...
def negSamplingLossAndGradient(
    ...
    negSamplesVal = np.array([-np.log(sigmoid(-np.dot(outsideVectors[val], centerWordVec))) for val in indices[1:]])
    loss = -np.log(sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))) + np.sum(negSamplesVal)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = np.dot(centerWordVec, (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))-1))
    
    for i in range(K):
        w_c = indices[i+1]
        gradOutsideVecs[w_c] += np.dot(1-sigmoid(-np.dot(outsideVectors[w_c], centerWordVec)), centerWordVec)
        
    gradCenterVec = np.dot((sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))-1), outsideVectors[outsideWordIdx]) + sum([-np.dot(sigmoid(-np.dot(outsideVectors[val], centerWordVec))-1, outsideVectors[val]) for val in indices[1:]])
    ...
    print(gradOutsideVecs[indices[1]])
    assert 1==2'''
